...
export function setToken(data) {
  ...
  Cookies.set(AccessToken, data.access_token, { expires: new Date(parsedToken.exp * 1000) })
  Cookies.set(RefreshToken, data.refresh_token, { expires: new Date(parsedToken.exp_rf * 1000) })
...
export function checkUserRole(res, to, next) {
  if (whiteList.includes(to.name)) {
    next()
  } else if (to.meta.group && res.data.groups.find((group) => to.meta.group.includes(group))) {
    next()
  } else {
